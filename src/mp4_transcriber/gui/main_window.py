...
class TranscriptionWorker(QRunnable):
    ...
    """
    def __init__(self, file_path, output_dir, model, include_timestamps, auto_clean, keep_audio):
        super().__init__()
    ...
    def run(self):
        ...
        try:
            ...
            self.signals.log.emit(f"Started processing {os.path.basename(self.file_path)}")
            ...
            self.signals.log.emit(f"Error processing {os.path.basename(self.file_path)}: {str(e)}")
...
class MP4TranscriberGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.files_table = QTableWidget(0, 3)  # Rows will be added dynamically, 3 columns
        ...
        default_output = os.environ.get('DEFAULT_OUTPUT_LOCATION', os.path.expanduser("~/Downloads"))
    ...
    def add_files(self):
        """Open file dialog to add MP4 files to the queue"""
        # Get default input location from environment or use fallback
        default_input = os.environ.get('DEFAULT_INPUT_LOCATION', os.path.expanduser("~/Movies"))
        ...
        if files:
            ...
            self.log_message(f"Added {len(files)} file(s) to queue")
    ...
    def stop_processing(self):
        ...
        if self.current_worker and hasattr(self.current_worker, 'processor') and self.current_worker.processor:
            try:
    ...
    def on_worker_error(self, file_path, error_msg):
        ...
        if "terminated" not in error_msg.lower() and "cancelled" not in error_msg.lower():
            # Find the file in the queue
            for i, file_info in enumerate(self.file_queue):
                if file_info["path"] == file_path:
            ...
            self.log_message(f"Error processing {os.path.basename(file_path)}: {error_msg}")
